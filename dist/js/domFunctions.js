...
export const updateDisplay = (weatherJson, locationObj) => {
  ...
  const screeenReaderWeather = buildScreenReaderWeather(
    weatherJson,
    locationObj
  );
  updateScreenReaderConfirmation(screeenReaderWeather);
...
const getWeatherClass = (icon) => {
  const firstTwoChars = icon.slice(2);
...
const setBGImage = (weatherClass) => {
  ...
  docunment.documentElement.classList.forEach((img) => {
...
const setFocusOnSearch = () => {
  docunment.getElementById("searchBar__text").focus();
};
...
const createCurrentConditionsDivs = (weatherObj, unit) => {
  ...
  const desc = createElem("div", "icon", properDesc);
...
const translateIconToFontAwesome = (icon) => {
  const i = document.createElememt("i");
  ...
  switch (firstTwoChars) {
    ...
    case "11":
      ...
      break;
    case "13":
      i.classList.add("far", "fa-cloud-rain");
      break;
    case "50":
